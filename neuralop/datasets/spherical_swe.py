...
def load_spherical_swe(
    ...
    train_dataset = SphericalSWEDataset(
        dims=train_resolution, num_examples=n_train, device=device
    )
...
class SphericalSWEDataset(paddle.io.DataLoader):
    ...
    def __getitem__(self, index):
        ...
        with paddle.no_grad():
            ...
            if self.normalize:
                inp = (inp - self.inp_mean) / paddle.sqrt(self.inp_var)
                tar = (tar - self.inp_mean) / paddle.sqrt(self.inp_var)
