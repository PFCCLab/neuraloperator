...
def test_sht(grid):
    ...
    coeffs[:, :lmax, :mmax] = paddle.randn([batch_size, lmax, mmax], dtype=paddle.complex64)

    signal = sht_handle.isht(coeffs, s=(nlat, nlon), grid=grid, norm=norm).to(paddle.float32)
