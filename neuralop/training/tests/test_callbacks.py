...
def test_model_checkpoint_saves():
    ...
    for file_ext in ['model_state_dict.pdmodel', 'model_metadata.pkl', 'optimizer.pdopt', 'scheduler.pdopt']:
...
def test_model_checkpoint_and_resume():
    ...
    trainer = Trainer(
        ...
        callbacks=[
            CheckpointCallback(
                    save_dir=save_pth, save_optimizer=True, save_scheduler=True, save_best='h1'
            )  # monitor h1 loss
        ]
    )
    ...
    for file_ext in ['best_model_state_dict.pdmodel', 'best_model_metadata.pkl', 'optimizer.pdopt', 'scheduler.pdopt']:
        file_pth = save_pth / file_ext
        assert file_pth.exists()
...
def test_load_from_checkpoint():
    ...
    loaded_model_eval_errors = trainer.evaluate(loss_dict=eval_losses, data_loader=test_loader)
